  ...
  return (
    <div className="glass-panel p-3 h-full">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h3 className="text-[11px] font-bold text-foreground tracking-wider">COMMODITIES</h3>
        <span className="text-[9px] text-muted-foreground ml-auto">{data.length} assets</span>
      </div>
        ...
        {data.map((c, i) => (
          <div key={i} className="flex items-center justify-between py-1.5 px-2 rounded bg-card/50 border border-border/20 hover:border-primary/30 transition-colors">
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-bold text-muted-foreground w-8">{c.symbol}</span>
            ...
            <div className="flex items-center gap-3">
              ...
              <span className={`text-[10px] font-bold flex items-center gap-0.5 ${c.change > 0 ? 'text-emerald-400' : c.change < 0 ? 'text-red-400' : 'text-muted-foreground'}`}>
