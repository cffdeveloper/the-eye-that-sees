    ...
    return (
      <div className="glass-panel p-4">
        ...
        <div className="flex items-center gap-2 py-4">
          ...
          <span className="text-xs text-muted-foreground">Fetching latest news...</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="glass-panel p-4">
      <h2 className="text-xs font-bold text-foreground mb-3 flex items-center gap-1.5">
        <Newspaper className="w-3.5 h-3.5 text-primary" /> LIVE NEWS FEED
        <span className="text-[8px] text-muted-foreground ml-auto">{articles.length} articles</span>
      </h2>
      <div className="space-y-2 max-h-96 overflow-y-auto">
        {articles.map((article, i) => (
          ...
          >
            ...
            {article.summary && (
              <div className="text-[9px] text-muted-foreground mt-1 leading-relaxed min-w-0 [&_p]:mb-0">
                <InlineMarkdown content={article.summary} />
              </div>
            )}
            <div className="flex items-center gap-2 mt-1.5">
              {article.source && (
                <span className="text-[8px] px-1.5 py-0.5 rounded bg-primary/10 text-primary">{article.source}</span>
              )}
              {article.publishedAt && (
